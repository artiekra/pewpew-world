...
const ARCHIVE_START_DATE = new Date(2025, 11, 8); // December 8th, 2025
const ARCHIVE_START_DAY = 8;
const ARCHIVE_START_MONTH = 11;
  ...
  const fetchUptimeData = async (year: number, month: number) => {
    try {
      const response = await api.get<UptimeResponse>(
        `/v1/archive/uptime/blitz_leaderboard/${year}/${month}`,
      );
