...
def run():
    ...
    last_blitz_timestamp = player_changes.get("_metadata", {}).get("last_blitz_processed", 0)
    ...
    if os.path.exists(xp_archive_dir):
        for filename in sorted(os.listdir(xp_archive_dir)):
            if filename.startswith("xp_lb_") and filename.endswith(".json"):
                ...
                last_xp_timestamp = process_xp_archive(filepath, player_changes, last_xp_timestamp)
    ...
    if os.path.exists(blitz_archive_dir):
        for filename in sorted(os.listdir(blitz_archive_dir)):
            if filename.startswith("blitz_lb_") and filename.endswith(".json"):
                ...
                last_blitz_timestamp = process_blitz_archive(filepath, player_changes, last_blitz_timestamp)
    ...
    logger.info(f"Player data tracking completed. Processed XP up to {last_xp_timestamp}, Blitz up to {last_blitz_timestamp}")
    ...
    for entry in entries:
        ...
        for player in data:
            ...
            if acc not in player_changes:
                player_changes[acc] = {"usernames": [], "xp_changes": [], "blitz_changes": []}
